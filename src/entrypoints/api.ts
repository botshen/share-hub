...
export function extractTweetUnion(tweet: TweetUnion): Tweet | null {
  try {
    ...
    if (tweet.__typename === 'TweetTombstone') {
      console.warn(`TweetTombstone received (Reason: ${tweet.tombstone?.text?.text})`, tweet);
      return null;
    }

    if (tweet.__typename === 'TweetUnavailable') {
      console.warn('TweetUnavailable received (Reason: unknown)', tweet);
      return null;
    }
...
export function filterEmptyTweet(tweet: Tweet): Tweet | null {
  if (!tweet.legacy) {
    console.warn('Empty tweet received', tweet);
    return null;
  }
